...
export class Socket{
    ...
    acceptFriendRequest(){
        this.socket.emit('acceptFriendRequest',{player: this.player,friend: this.friend});
    }
    friendChoseTigerGoat(item){
        this.socket.emit('friendChoseItem',{item, friend: this.friend,player: this.player,friendItem: item==TIGER ? GOAT: TIGER});
    }
    sendMoveDataToFriend(data,movedItem){
        this.socket.emit('friendMovedItem',{player: this.player,friend: this.friend,data: data,movedItem})
    }
}